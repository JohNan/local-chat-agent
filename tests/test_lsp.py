...
def mock_registry():
    ...
    LSPRegistry._config = {}
    with patch("builtins.open", mock_open(read_data=json.dumps(MOCK_CATALOG))), \
...
def test_registry_missing_binary():
    ...
    with patch("builtins.open", mock_open(read_data=json.dumps(MOCK_CATALOG))), \
         patch("os.path.exists", return_value=True), \
         patch("shutil.which", side_effect=lambda x: None): # Binary not found
        registry = LSPRegistry()
...
def test_lsp_manager_start_server(MockRegistry, MockPopen, mock_lsp_manager):
    ...
    with patch("app.services.lsp_manager.LSPServer") as MockLSPServer:
        ...
        server_instance.send_request.return_value = {"result": {"capabilities": {}}} # Success response
