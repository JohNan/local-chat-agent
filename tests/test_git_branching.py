...
def test_get_branches(mock_subprocess_run):
...
def test_switch_branch_success(mock_subprocess_run):
    mock_subprocess_run.return_value = MagicMock(
...
def test_switch_branch_failure(mock_subprocess_run):
...
async def test_api_switch_branch():
    with patch("app.services.git_ops.switch_branch", return_value={"success": True, "output": "ok"}):
